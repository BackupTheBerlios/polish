...
//#endif
{
	...
	private Hashtable menuItemsByCommand;
	private Command cmdBackOrExit;
	...
	{
		//#if !polish.useFullScreen
		if (this.menuItemsByCommand == null) {
			this.menuItemsByCommand = new Hashtable();
		}
		CommandMenuItem item = new CommandMenuItem(cmd, this );
		this.menuItemsByCommand.put(cmd, item);
		addMenuItem( item );
		setBackOrExitCommand(cmd);
		 //#endif
	}
	
	//#if !polish.useFullScreen
	  public boolean onClose() {
	        if (!this.isClosing && this.cmdBackOrExit != null) {
	            this.isClosing = true;
	            handleCommand(this.cmdBackOrExit);
	...
	public void removeCommand( Command cmd)
	{
		//#if !polish.useFullScreen
		if (this.menuItemsByCommand != null) {
			MenuItem item = (MenuItem) this.menuItemsByCommand.remove(cmd);
			if (cmd == this.cmdBackOrExit) {
				this.cmdBackOrExit = null;
				Enumeration keys = this.menuItemsByCommand.keys();
				while (keys.hasMoreElements()) {
					Command cmdBack = (Command) keys.nextElement();
					setBackOrExitCommand( cmdBack );
				}
			}
			if (item != null) {
				removeMenuItem(item);
	...
	private void setBackOrExitCommand(Command cmd) {
		//#if !polish.useFullScreen
			int type = cmd.getCommandType();
			 if ((type == Command.BACK || type == Command.EXIT || type == Command.CANCEL)
					 && (this.cmdBackOrExit == null || this.cmdBackOrExit.getPriority() > cmd.getPriority())) 
			 {
	             this.cmdBackOrExit = cmd;
	         }		
		//#endif
	}
