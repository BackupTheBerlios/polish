 ...
 *
 */
public class MaskedGradientVerticalBackgroundConverter extends BackgroundConverter {
	...
	protected String createNewStatement(HashMap map, Style stlye,
		...
		String maskColor = "0xFFFFFF";
		String maskColorStr = (String) map.get("mask-color");
		if ( maskColorStr != null ) {
			maskColor = parseColor( maskColorStr );			
		}
		
		String singleGradientStr = (String) map.get("single-gradient");
		...
		String xOffsetStr = (String) map.get("x-offset");
		if (xOffsetStr == null) {
			xOffsetStr = "0";
		} else {
			parseInt("x-offset", xOffsetStr);
		}
		String yOffsetStr = (String) map.get("y-offset");
		if (yOffsetStr == null) {
			yOffsetStr = "0";
		} else {
		...
		String result = "new de.enough.polish.ui.backgrounds.MaskedGradientVerticalBackground(" 
			+  "\"" + mask + "\", " + maskColor + ", " + singleGradientStr + ","
			+ firstTopColor + ", " + firstBottomColor + "," + secondTopColor + ", " + secondBottomColor
			+ "," + xOffsetStr + "," + yOffsetStr + "," + anchor   
			+ "," + start + "," + end + "," + isPercent + ")";
