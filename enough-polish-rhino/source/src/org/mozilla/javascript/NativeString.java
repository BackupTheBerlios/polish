 ...
 * @author Norris Boyd
 */
final public class NativeString extends IdScriptableObject
{
    ...
    {
        if (id == Id_length) {
            return ScriptRuntime.wrapNumber(string.length());
        }
          ...
          case Id_replace:
            ...
            case Id_localeCompare:
            {
                ...
                return ScriptRuntime.wrapNumber(
                        ScriptRuntime.toString(thisObj).compareTo( 
                        ScriptRuntime.toString(args, 0)));
            }
    ...
    private static int js_lastIndexOf(String target, Object[] args) {
        ...
        return  Util.lastIndexOf(target, search, (int)end);
//        return  target.lastIndexOf(search, (int)end);
                                  ...
                                  String[][] parensp)
    {
            ...
            re == null && separator.length() == 1 && separator.charAt(0) == ' ')
        {
            ...
            if (i == 0) {
                while (i < length && SFCharacter.isWhitespace(target.charAt(i)))
                    i++;
                ip[0] = i;
            }
            ...
            while (i < length
                   && !SFCharacter.isWhitespace(target.charAt(i)))
                i++;

            /* Now skip the next run of whitespace. */
            int j = i;
            while (j < length && SFCharacter.isWhitespace(target.charAt(j)))
                j++;
...
// #/string_id_map#
	...
	public void read(DataInputStream in) throws IOException {
		super.read(in);
		string = (String) Serializer.deserialize(in);
	}
