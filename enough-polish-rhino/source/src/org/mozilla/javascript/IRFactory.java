...
final class IRFactory
{
    ...
    private Node createMemberRefGet(Node target, String namespace, Node elem,
                                    int memberTypeFlags)
    {
        return new Node(Token.GET_REF, elem);
    }
    ...
    Node createBinary(int nodeType, Node left, Node right)
    {
        switch (nodeType) {
          ...
          case Token.ADD:
            ...
            if (left.nType == Token.STRING) {
                String s2;
                if (right.nType == Token.STRING) {
                    s2 = right.getString();
                } else if (right.nType == Token.NUMBER) {
                ...
                return left;
            } else if (left.nType == Token.NUMBER) {
                if (right.nType == Token.NUMBER) {
                    left.setDouble(left.getDouble() + right.getDouble());
                    return left;
                } else if (right.nType == Token.STRING) {
                    String s1, s2;
