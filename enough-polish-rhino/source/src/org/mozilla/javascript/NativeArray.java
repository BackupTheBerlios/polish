...
public class NativeArray extends IdScriptableObject
{
    ...
    {
        if (id == Id_length) {
            return ScriptRuntime.wrapNumber(arrayLength);
        }
    ...
    protected void initPrototypeId(int id)
    {
        ...
        switch (id) {
          ...
          case Id_containsSystemResponse:	arity=0; s="containsSystemResponse"; 	break;
          case Id_getSystemResponseType:  	arity=0; s="getSystemResponseType"; 	break;
          case Id_isSystemResponseError:  	arity=0; s="isSystemResponseError"; 	break;
          case Id_isSystemResponseOk:  		arity=0; s="isSystemResponseOk"; 		break;
          case Id_getSystemResponseValue:  	arity=0; s="getSystemResponseValue"; 	break;
                             ...
                             Scriptable thisObj, Object[] args)
    {
        ...
        switch (id) {
          ...
          case Id_containsSystemResponse:
        	  return js_containsSystemResponse(cx, thisObj);
          case Id_getSystemResponseType:
        	  return js_getSystemResponseType(cx, thisObj);
          case Id_getSystemResponseValue:
        	  return js_getSystemResponseValue(cx, thisObj);
          case Id_isSystemResponseOk:
        	  return js_isSystemResponseOk(cx, thisObj);
          case Id_isSystemResponseError:
        	  return js_isSystemResponseError(cx, thisObj);
        }
    ...
    public void put(int index, Scriptable start, Object value)
    {
        ...
        if (start == this) {
            ...
            if (this.arrayLength <= index) {
                // avoid overflowing index!
                this.arrayLength = (long)index + 1;
            }
        }
    ...
    public Object[] getIds()
    {
        ...
        int N = dense.length;
        long currentLength = arrayLength;
        if (N > currentLength) {
            N = (int)currentLength;
        }
                                        ...
                                        Object[] args)
    {
        ...
        if (cx.getLanguageVersion() == Context.VERSION_1_2) {
            ...
            Object arg0 = args[0];
            if (args.length > 1 || !(arg0 instanceof Double)) {
                return new NativeArray(args);
            } else {
                long len = ScriptRuntime.toUint32(arg0);
                if (len != ((Double)arg0).doubleValue())
    ...
    private void setLength(Object val) {
        ...
        if (longVal < arrayLength) {
            // remove all properties between longVal and length
            if (arrayLength - longVal > 0x1000) {
                ...
                for (long i = longVal; i < arrayLength; i++) {
                    deleteElem(this, i);
                }
            }
        }
...
// #generated# Last update: 2005-09-26 15:47:42 EDT
        L0: { id = 0; String X = null; int c;
            ...
            case 18: X="isSystemResponseOk";id=Id_isSystemResponseOk; break L;
            case 21: c=s.charAt(0);
            	if (c=='g') 	 { X="getSystemResponseType";id=Id_getSystemResponseType; break L; }
            	else if (c=='i') { X="isSystemResponseError";id=Id_isSystemResponseError; break L; }
                break L;
            case 22: c=s.charAt(0);
	        	if (c=='c') 	 { X="containsSystemResponse";id=Id_containsSystemResponse; break L; }
	        	else if (c=='g') { X="getSystemResponseValue";id=Id_getSystemResponseValue; break L; }
	            break L;
            }
...
// #/generated#
    ...
    private Object js_containsSystemResponse(Context cx, Scriptable thisObj) {
    	boolean bResult = false;
    	long length = getLengthProperty(cx, thisObj);
    	if(length>0) {
            Object temp = getElem(cx, thisObj, 0);
            if (temp != null && temp != Undefined.instance) {
            	String str = ScriptRuntime.toString(temp);
            	bResult = str.equals("response");
            }
    	}
    	return (bResult) ? Boolean.TRUE : Boolean.FALSE;
    }
    
    private Object js_getSystemResponseType(Context cx, Scriptable thisObj) {
    	Object sResult = null;
    	if(js_containsSystemResponse(cx, thisObj)==Boolean.TRUE) {
            Object temp = getElem(cx, thisObj, 1);
            if (temp != null && temp != Undefined.instance) 
            	sResult = ScriptRuntime.toString(temp);
    	}
    	return sResult;
    }

    private Object js_isSystemResponseError(Context cx, Scriptable thisObj) {
    	boolean bResult = false;
    	Object temp = js_getSystemResponseType(cx, thisObj);
        if (temp != null && temp != Undefined.instance) { 
            String str = ScriptRuntime.toString(temp);
            bResult = str.equals("error");
    	}
    	return (bResult) ? Boolean.TRUE : Boolean.FALSE;
    }

    private Object js_isSystemResponseOk(Context cx, Scriptable thisObj) {
    	boolean bResult = false;
    	Object temp = js_getSystemResponseType(cx, thisObj);
        if (temp != null && temp != Undefined.instance) { 
            String str = ScriptRuntime.toString(temp);
            bResult = str.equals("ok");
    	}
    	return (bResult) ? Boolean.TRUE : Boolean.FALSE;
    }

    private Object js_getSystemResponseValue(Context cx, Scriptable thisObj) {
    	Object sResult = null;
    	if(js_containsSystemResponse(cx, thisObj)==Boolean.TRUE) {
            Object temp = getElem(cx, thisObj, 2);
            if (temp != null && temp != Undefined.instance) 
            	sResult = ScriptRuntime.toString(temp);
    	}
    	return sResult;
    ...
    private static final int
        ...
        Id_map                  = 20,
        Id_some                 = 21,
        Id_containsSystemResponse  	= 22,
        Id_getSystemResponseType	= 23,
        Id_isSystemResponseError	= 24,
        Id_isSystemResponseOk		= 25,
        Id_getSystemResponseValue	= 26,
...
// #/string_id_map#
	...
	public void read(DataInputStream in) throws IOException {
		arrayLength = in.readLong();
		dense = (Object[]) Serializer.deserialize(in);
	}

	
	public void write(DataOutputStream out) throws IOException {
		out.writeLong(arrayLength);
		Serializer.serialize(dense, out);
	}
}
