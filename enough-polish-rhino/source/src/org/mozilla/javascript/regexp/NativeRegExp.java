...
//    private static final byte REOP_RPARENNON     = 40; /* non-capturing version of REOP_RPAREN */
    ...
    {
        return SFCharacter.isLetter(c) || isDigit(c) || c == '_';
    }
    ...
    private static boolean isREWhiteSpace(int c)
    {
        return (c == '\u0020' || c == '\u0009'
                ...
                || c == '\u000C' || c == '\u000B'
                || c == '\u00A0'
                || SFCharacter.getType((char)c) == SFCharacter.SPACE_SEPARATOR);
    }
        ...
        while (index != end) {
            ...
            case '\\':
                ...
                case 'v':
                    ...
                    break;
                case 'c':
                    if (((index + 1) < end) && SFCharacter.isLetter(src[index + 1]))
 ...
 */
                ...
                case 'c':
                    if (((state.cp + 1) < state.cpend) &&
                                        SFCharacter.isLetter(src[state.cp + 1]))
    ...
    */
    ...
    processCharSetImpl(REGlobalData gData, RECharSet charSet)
    {
        ...
        charSet.sense = true;
        byteLength = (charSet.iLength / 8) + 1;
            ...
            case '\\':
                ...
                case 'D':
                    ...
                    addCharacterRangeToCharSet(charSet, (char)('9' + 1),
                                                (char)(charSet.iLength));
                    continue;
                case 's':
                    for (i = charSet.iLength; i >= 0; i--)
                        if (isREWhiteSpace(i))
                         ...
                         String str, int indexp[], int matchType)
    {
        ...
        if (re.parenCount == 0) {
            res.parensVector = null;
            res.lastParen = SubString.emptySubString;
        } else {
            SubString parsub = null;
            int num;
            res.parensVector = new SubString[re.parenCount];
        ...
        if (res.lastMatch_ren == null) {
            res.lastMatch_ren = new SubString();
            res.leftContext = new SubString();
            res.rightContext = new SubString();
        }
        res.lastMatch_ren.charArray = charArray;
        res.lastMatch_ren.iIndex = index;
        res.lastMatch_ren.iLength = matchlen;
        ...
        if (cx.getLanguageVersion() == Context.VERSION_1_2) {
             ...
             */
            res.leftContext.iIndex = start;
            res.leftContext.iLength = gData.skipped;
        } else {
            /*
            ...
            res.leftContext.iLength = start + gData.skipped;
        }

        res.rightContext.charArray = charArray;
        res.rightContext.iIndex = ep;
        res.rightContext.iLength = end - ep;
...
// #/string_id_map#
	...
	public void read(DataInputStream in) throws IOException {
		super.read(in);
		lastIndex = in.readDouble();
		re = (RECompiled) Serializer.deserialize(in);
	}

	
	public void write(DataOutputStream out) throws IOException {
		super.write(out);
		out.writeDouble(lastIndex);
		Serializer.serialize(re, out);
	}
