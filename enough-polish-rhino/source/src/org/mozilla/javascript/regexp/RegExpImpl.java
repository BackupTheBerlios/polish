...
public class RegExpImpl implements RegExpProxy {
          ...
          case RA_REPLACE:
            {
                ...
                if (data.charBuffer == null) {
                    ...
                    replace_glob(data, cx, scope, this, lc.iIndex, lc.iLength);
                }
                data.charBuffer.append(rc.charArray, rc.iIndex, rc.iLength);
                return data.charBuffer.toString();
            }
                                         ...
                                         GlobData data, boolean forceFlat)
    {
        ...
        if (data.mode == RA_SEARCH) {
            ...
            if (result != null && result.equals(Boolean.TRUE))
                result = new Integer(reImpl.leftContext.iLength);
            else
            ...
            for (int count = 0; indexp[0] <= str.length(); count++) {
                ...
                if (data.mode == RA_MATCH) {
                    ...
                    SubString lastMatch = reImpl.lastMatch_ren;
                    int leftIndex = data.iLeftIndex;
                    int leftlen = lastMatch.iIndex - leftIndex;
                    data.iLeftIndex = lastMatch.iIndex + lastMatch.iLength;
                    replace_glob(data, cx, scope, reImpl, leftIndex, leftlen);
                }
                if (reImpl.lastMatch_ren.iLength == 0) {
                          ...
                          boolean[] matched, String[][] parensp)
    {
        ...
        while (true) {  // imitating C label
            ...
            SubString sep = this.lastMatch_ren;
            matchlen[0] = sep.iLength;
            if (matchlen[0] == 0) {
                /*
        ...
        int size = (parensVector == null) ? 0 : parensVector.length;
    ...
    {
        if (parensVector != null && i < parensVector.length) {
            SubString parsub = parensVector[i];
            if (parsub != null) {
                return parsub;
            }
        }
                                     ...
                                     int leftIndex, int leftlen)
    {
        ...
        if (rdata.fLambda != null) {
            ...
            args[parenCount+1] = new Integer(reImpl.leftContext.iLength);
            args[parenCount+2] = rdata.strstr;
            ...
            lambdaStr = null;
            replen = rdata.repstr_ren.length();
            if (rdata.dollar >= 0) {
                int[] skip = new int[1];
                int dp = rdata.dollar;
                do {
                    SubString sub = interpretDollar(cx, reImpl, rdata.repstr_ren,
                                                    dp, skip);
                    if (sub != null) {
                        replen += sub.iLength - skip[0];
                        dp += skip[0];
                    } else {
                        ++dp;
                    }
                    dp = rdata.repstr_ren.indexOf('$', dp);
                } while (dp >= 0);
            }
        }

        int growth = leftlen + replen + reImpl.rightContext.iLength;
        StringBuffer charBuf = rdata.charBuffer;
        if (charBuf == null) {
            charBuf = new StringBuffer(growth);
            rdata.charBuffer = charBuf;
        } else {
            charBuf.ensureCapacity(rdata.charBuffer.length() + growth);
        }
                                             ...
                                             String da, int dp, int[] skip)
    {
        ...
        if (NativeRegExp.isDigit(dc)) {
                ...
                && version <= Context.VERSION_1_4)
            {
                ...
                int parenCount = (res.parensVector == null) ? 0 : res.parensVector.length;
                num = dc - '0';
                if (num > parenCount)
                    return null;
                cp = dp + 2;
        ...
        switch (dc) {
          ...
          case '`':
            if (version == Context.VERSION_1_2) {
                 ...
                 */
                res.leftContext.iIndex = 0;
                res.leftContext.iLength = res.lastMatch_ren.iIndex;
            }
        ...
        if (dp != -1) {
            ...
            do {
                ...
                if (sub != null) {
                    len = sub.iLength;
                    if (len > 0) {
                        charBuf.append(sub.charArray, sub.iIndex, len);
                    }
                    cp += skip[0];
                    dp += skip[0];
                } else {
    ...
    SubString[]     parensVector;        /* Vector of SubString; last set of parens
                                      matched (perl $1, $2) */
    SubString       lastMatch_ren;     /* last string matched (perl $&) */
...
final class GlobData
{
    ...
    String   strstr;       /* output: 'this' parameter object as string */
    ...
    Function      fLambda;        /* replacement function object or null */
    String        repstr_ren;        /* replacement string */
    int           dollar = -1;   /* -1 or index of first $ in repstr */
    StringBuffer  charBuffer;       /* result characters, null initially */
    int           iLeftIndex;     /* leftContext index, always 0 for JS1.2 */
}
