...
import org.mozilla.javascript.util.SFCharacter;
...
public class NativeGlobal implements Externalizable, IdFunctionCall
{
...
//                  break;
              case Id_exitWidget:
                  name = "exitWidget";
                  break;
              case Id_registerOverviewTemplate:
                  name = "registerOverviewTemplate";
                  break;
              case Id_deleteOverviewTemplate:
                  name = "deleteOverviewTemplate";
                  break;
              case Id_platformRequest:
        ...
        String[] errorMethods = TextUtil.split (
                                    "ConversionError;"
                                    +"EvalError;"
                                    +"RangeError;"
                                    +"ReferenceError;"
                             ...
                             Scriptable thisObj, Object[] args)
    {
        if (f.hasTag(FTAG)) {
            ...
            switch (methodId) {
                ...
                case Id_exitBase:
                	EventManager.fireEvent("js.exit", this, null);
                	return "END";
//                case Id_setProperties:
//                    setProperties(args, cx.currentWidgetId);
//                    return null;
//                case Id_getProperties:
//                    return getProperties(cx, scope, cx.currentWidgetId);
                case Id_trace:
...
//                    return null;
                ...
                case Id_sleep:
                	...
                	return null;
//                case Id_registerOverviewTemplate:
//                    js_registerOverviewTemplate(cx, args);
//                    return null;
//                case Id_deleteOverviewTemplate:
//                    js_deleteOverviewTemplate(cx, args);
//                    return null;
                case Id_platformRequest:
                    platformRequest(cx, args);
                    return null;
            }
        }
        throw f.unknown();
...
//	 * @param args
//	 */
//	private void runWidget(Context context, Object[] args) {
//		int wId = ScriptRuntime.toInt32(args, 0);
//		context.setNextWidget(wId);
//	}
    ...
    private Object js_parseFloat(Object[] args)
    {
        ...
        if (c == 'I') {
            // check for "Infinity"
            if (i+8 <= len && s.regionMatches(false, i, "Infinity", 0, 8)) {
                double d;
                if (s.charAt(start) == '-') {
                    d = Double.NEGATIVE_INFINITY;
                } else {
...
//    private static void setProperties(Object[] args, int widgetID)
//    {
...
//        	dos.writeInt(widgetID);
//        	dos.writeByte(NativeRecordstore.RSTYPE_BOOLEAN);
//        	dos.writeBoolean(true);
//        	dos.write(Util.getRecordFromArray((NativeArray) args[0], null));
//        	int rId = RmsHandler.getRecordId(RmsHandler.WPROPS, widgetID);
//        	rs = RecordStore.openRecordStore(RmsHandler.WPROPS, true);
//        	byte[] record = bos.toByteArray();
//        	if (rId != -1) {
//        		rs.setRecord(rId, record, 0, record.length);
//        	} else {
...
//	        throw NativeRecordstore.createRecordStoreError(e.getMessage());
//		} catch (IOException e) {
//			throw ScriptRuntime.typeError("msg.props.corrupt");
//		} finally {
//			if (rs != null) {
//				try {
//					rs.closeRecordStore();
...
//    }
//
//    private Object getProperties(Context cx, Scriptable scope, int widgetID) {
//		try {
//			byte[] record = RmsHandler.getRecord(RmsHandler.WPROPS, widgetID);
//			
...
//			dis.readByte();
//			dis.readBoolean();
//			return Util.createArrayFromRecord(cx, scope, dis, false);
//		} catch (RmsException e) {
//	        throw NativeRecordstore.createRecordStoreError(e.getMessage());
//		} catch (IOException e) {
//			throw ScriptRuntime.typeError("msg.props.corrupt");
...
//	}
    ...
    private void js_sleep(Object[] args) {
    	long millis = 0L;
    	if(args.length>0)
    		millis = ScriptRuntime.toUint32(args[0]);
    	try {
			Thread.sleep(millis);
...
//    private static void js_registerOverviewTemplate(Context cx, Object[] args) {
//        int widgetId = cx.currentWidgetId;
//        if (widgetId==0 || args == null || args.length <= 2) { 
//            throw NativeRecordstore.createRecordStoreError("Invalid Arguments");
//        }
//        int templateNumber = ScriptRuntime.toInt32(args[0]);
//        int templateType = ScriptRuntime.toInt32(args[1]);
//        String template    = ScriptRuntime.toString(args[2]);
//        
//        RecordStore rsOverviewTemplates = null;
//        try {
//            rsOverviewTemplates = RecordStore.openRecordStore(RmsHandler.WOVTEMPLATES, true);
//            ByteArrayOutputStream baos = new ByteArrayOutputStream();
//            DataOutputStream dos = new DataOutputStream(baos);
//            try {
//                dos.writeInt(widgetId);         // WidgetId
//                dos.writeInt(templateNumber);   // vom Widget vergebene Template Nummer
//                dos.writeInt(templateType);     // Template-Typ
//                dos.writeUTF(template);         // Template
//            } catch (IOException e) {
//                e.printStackTrace();
//            } finally {                 
//                try {
...
//                }
//            }
//            rsOverviewTemplates.addRecord(baos.toByteArray(), 0, baos.size());
...
//        } finally {
//            try {
//                rsOverviewTemplates.closeRecordStore();
...
//    }
//    
//    private static void js_deleteOverviewTemplate(Context cx, Object[] args) {
//        int widgetId = cx.currentWidgetId;
//        if (widgetId==0 || args == null || args.length <= 0) { 
//            throw NativeRecordstore.createRecordStoreError("Invalid Arguments");
//        }
//        int templateNumber = ScriptRuntime.toInt32(args[0]);
//        RecordStore rsOverviewTemplates = null;
//        try {
//            rsOverviewTemplates = RecordStore.openRecordStore(RmsHandler.WOVTEMPLATES, true);
//            RecordEnumeration e = rsOverviewTemplates.enumerateRecords(null, null, false);
//            int recNo;
//            while (e.hasNextElement()) {
//                recNo = e.nextRecordId();
//                byte[] aRec = rsOverviewTemplates.getRecord(recNo);
//                if (aRec != null) {
//                    DataInputStream dis = new DataInputStream(new ByteArrayInputStream(aRec));
//                    try {
//                        int rsId = dis.readInt();
//                        int tN   = dis.readInt();
//                        if ( rsId==widgetId && tN == templateNumber) {
//                            rsOverviewTemplates.deleteRecord(recNo);
//                        }
//                    } catch (IOException e1) {
//                        throw NativeRecordstore.createRecordStoreError(e1.getMessage());
//                    } finally {
//                        try {
//                            dis.close();
//                        } catch (IOException e1) {
...
//    }
	...
	private void platformRequest(Context cx, Object[] args) {
        if (args == null || args.length == 0) {
        	args = ScriptRuntime.padArguments(args, 1);
        }
    	String url = ScriptRuntime.toString(args, 0);
		EventManager.fireEvent("js.platformRequest", this, url);
	}
...
//        Id_runWidget   		   = 17,
        Id_exitWidget   	   = 17,
        Id_registerOverviewTemplate = 18,
        Id_deleteOverviewTemplate = 19,
        Id_platformRequest = 20,
        
        LAST_SCOPE_FUNCTION_ID = 20,

        Id_new_CommonError     = 21;
