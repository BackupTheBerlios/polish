 ...
 */
public class NativeWith implements Scriptable, IdFunctionCall, Externalizable {
    
    //pricvate static final long serialVersionUID = 1L;
    ...
    {
        if (f.hasTag(FTAG)) {
            if (f.getMethodId() == Id_constructor) {
    ...
    static boolean isWithFunction(Object functionObj)
    {
        if (functionObj instanceof IdFunctionObject) {
            ...
            return f.hasTag(FTAG) && f.getMethodId() == Id_constructor;
        }
        return false;
    }
	...
	public void read(DataInputStream in) throws IOException {
		prototype = (Scriptable) Serializer.deserialize(in);
		parent = (Scriptable) Serializer.deserialize(in);
		
	}

	
	public void write(DataOutputStream out) throws IOException {
		Serializer.serialize(prototype, out);
