 ...
 * the output string and malloc fewer bytes depending on d and base, but why bother? */
    ...
    static String JS_dtobasestr(int baseTo, double d)
    {
        if (!(2 <= baseTo && baseTo <= 36))
            throw new IllegalArgumentException("Bad base: "+baseTo);
        ...
        if (lfloor == dfloor) {
            // int part fits long
            intDigits = Long.toString((negative) ? -lfloor : lfloor, baseTo);
        } else {
...
//            JS_ASSERT(-s2 < e);
            ...
            BigInteger bigBase = BigInteger.valueOf(baseTo);
