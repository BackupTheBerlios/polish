...
// ObjToIntMap is a copy cat of ObjToIntMap with API adjusted to object keys
    ...
    public Object intern(Object keyArg) {
        ...
        theValues[index] = 0;
        return (nullKey) ? null : theKeys[index];
    }
    ...
    final void initIterator(Iterator i) {
        i.init(theKeys, theValues, keyCountMember);
    }
    ...
    private int findIndex(Object key) {
        if (theKeys != null) {
            ...
            Object test = theKeys[index];
            if (test != null) {
                int N = 1 << power;
                if (test == key
                    || (theValues[N + index] == hash && test.equals(key)))
                {
                    return index;
                }
...
// and enough free space
    private int insertNewKey(Object key, int hash) {
        if (check && occupiedCount != keyCountMember) Kit.codeBug();
        if (check && keyCountMember == 1 << power) Kit.codeBug();
        int fraction = hash * A;
        int index = fraction >>> (32 - power);
        int N = 1 << power;
        if (theKeys[index] != null) {
            ...
            do {
                if (check && theKeys[index] == DELETED) Kit.codeBug();
                index = (index + step) & mask;
                if (check && firstIndex == index) Kit.codeBug();
            } while (theKeys[index] != null);
        }
        theKeys[index] = key;
        theValues[N + index] = hash;
        ++occupiedCount;
        ++keyCountMember;
...
// Ensure key index creating one if necessary
    private int ensureIndex(Object key) {
        ...
        if (check && theKeys != null && theKeys[index] != null)
            Kit.codeBug();
        if (firstDeleted >= 0) {
            index = firstDeleted;
        }
            ...
            if (theKeys == null || occupiedCount * 4 >= (1 << power) * 3) {
...
// values[N <= i < 2*N]: hash code of key at keys[i-N]
    ...
    private transient Object[] theKeys;
    private transient int[] theValues;

    private int power;
    private int keyCountMember;
...
// If true, enables consitency checks
	...
	public void read(DataInputStream in) throws IOException {
		power = in.readInt();
		keyCountMember = in.readInt();
        int writtenKeyCount = keyCountMember;
        if (writtenKeyCount != 0) {
            ...
            for (int i = 0; i != writtenKeyCount; ++i) {
                Object key = Serializer.deserialize(in);
                int hash = key.hashCode();
                int index = insertNewKey(key, hash);
                theValues[index] = in.readInt();
            }
        }
	}
	...
	public void write(DataOutputStream out) throws IOException {
		out.writeInt(power);
		out.writeInt(keyCountMember);
        int count = keyCountMember;
        for (int i = 0; count != 0; ++i) {
            Object key = theKeys[i];
            if (key != null && key != DELETED) {
                --count;
                Serializer.serialize(key, out);
                out.writeInt(theValues[i]);
            }
        }
