...
public class LayoutModeler {
	...
	public static void text(Box box, int level) {
		...
		String text = "[ " + node.getHandler().getTag() + " " + " "
				+ box.clearLeft + " " + box.clearRight + " "
				+ box.hasDynamicSize + " D: "
				+ StyleManager.getProperty(box, "display") + " F: "
				+ StyleManager.getProperty(box, "float") + " X: " + box.x
				+ " Y: " + box.y + " M: " + box.marginLeft + " TW: "
				+ box.getTotalWidth() + " CW: " + box.contentWidth + " CH: "
				+ box.contentHeight + " C: " + node.getContent() + " ]";

		out(text, level);
	}

	/*
	 * public static void recParse(CssElement node, int level) { ArrayList kids
	 * = node.getChildren() ;
	 * 
	 * out ( text(node) , level );
	 * 
	 * if ( kids != null ) { int i = 0; while ( i < kids.size() ) { recParse(
	 * (CssElement) kids.get(i), level+1); i++; } } }
	 */
	...
	public static void paintBox(Box b, Graphics g) {
		...
		if (!"none".equals(StyleManager.getProperty(b, "float"))) {
			...
			g.fillRect(b.getAbsoluteX(), b.getAbsoluteY(), b.getTotalWidth(),
					b.getTotalHeight());
		}

		// Draw outer box margins
		g.setColor(0x11FF0000);
		g.drawRect(b.getAbsoluteX(), b.getAbsoluteY(), b.getTotalWidth(),
				b.getTotalHeight());

		// Draw inner box margins (actual content borders)
		g.setColor(0x1100FF00);
		g.drawRect(b.getAbsoluteX() + b.marginLeft, b.getAbsoluteY()
				+ b.marginTop,
				b.getTotalWidth() - b.marginLeft - b.marginRight,
				b.getTotalHeight() - b.marginTop - b.marginBottom);

		// Draw the actual content
		if (b.correspondingNode.getContentType() == CssElement.CONTENT_TEXT) {
			g.setFont(b.correspondingNode.getStyle().getFont());
			g.drawString((String) b.correspondingNode.getContent(),
					b.getAbsoluteX() + b.paddingLeft + b.marginLeft,
					b.getAbsoluteY() + b.paddingTop + b.marginTop, Graphics.TOP
							| Graphics.LEFT);
		} else if (b.correspondingNode.getContentType() == CssElement.CONTENT_IMAGE) {
			g.drawImage(((ImgCssElement) b.correspondingNode).getImage(),
					b.getAbsoluteX() + b.marginLeft + b.paddingLeft,
					b.getAbsoluteY() + b.marginTop + b.paddingTop, Graphics.TOP
							| Graphics.LEFT);
		}
		...
		if (b.isListItem && firstListItem != null) {
			g.setColor(255, 0, 0);
			g.fillRect(firstListItem.getAbsoluteX() - 10,
					firstListItem.getAbsoluteY() + 3, 5, 5);
		}
	...
	public Box model(CssElement rootNode, int contentWidth) {
		...
		this.floats = new FloatBoxManager();
		...
		LayoutContext context = new LayoutContext(myBox, this.floats);
		doModel(myBox, context);

		dumpBoxTree(myBox, 0);
		rootBox = myBox;

		System.out.println("FLOATS: " + this.floats.floats.size());
		return myBox;
	...
	protected void doModel(Box box, LayoutContext parentContext) {
		...
		LayoutContext context = new LayoutContext(box, this.floats);

		// In case this is a re-model operation for this box, clear all steps
		// from the previous modelling operation(s)
		context.removeFloatsForBox(box);
		box.removeAllChildren();

		// If I am an atomic element of the page (image, text, etc) just map my
		// attributes to my box and be done with me!
		...
		// Otherwise, figure out the total width of my children and then figure
		// out my width
		if (!"none".equals(floatMode)) {
			...
			if (d != null) {
				box.contentWidth = (d.getValue(box.parent.contentWidth))
						- box.marginLeft - box.marginRight - box.paddingLeft
						- box.paddingRight;
			} else {
				// No width specified, use maxwidth for now and figure out the
				// actual width later
				box.hasDynamicSize = true;
				box.contentWidth = box
						.getContentWidthFromTotalWidth(box.parent.contentWidth); // Temp
																					// value.
			}
		} else
		...
		if ("block".equals(displayMode) || "list-item".equals(displayMode)) {
			...
			if (d != null) {
				...
				box.contentWidth = box
						.getContentWidthFromTotalWidth(parentContext
								.getRemainingRowWidth());
				System.out.println("HOPA: " + box.contentWidth + " "
						+ parentContext.currentRowMaxWidth + " "
						+ parentContext.getCurrentX());
		...
		while (context.hasMoreChildren()) {
			...
			if (child.correspondingNode.getContentType() == CssElement.CONTENT_BR) {
				context.nextRow(box.correspondingNode.getStyle().getFont()
						.getHeight());
				continue;
			}
			...
			String childFloatMode = (String) StyleManager.getProperty(child,
					"float");
			String childDisplayMode = (String) StyleManager.getProperty(child,
					"display");

			// Handle the clear attribute
			if ((!"none".equals(childFloatMode))
					|| ("block".equals(childDisplayMode))) {

			}

			// If I am an inline-block element, or an inline float things get
			// tricky
			if ("inline-block".equals(childDisplayMode)
					|| (!"none".equals(childFloatMode))) {
				...
				// For floats, I should see if I can be placed on the current
				// row,
				// or if I must be saved for later so other elements can be
				// placed in my stead.
				// --------------------------------

				if (!"none".equals(childFloatMode)) {
					if (child.clearRight) {
						if ((context.getRightFloatsCount()) > 0
								|| (context
										.getCurrentRowRightFloatOccupiedSpace() > 0)) {
						...
						if (context.hasMoreRegularChildren()) {
							...
							do {
								context.nextRow(1);
									...
									.getCurrentRowLeftFloatOccupiedSpace() > 0));
						}
					}

					// Consider previous clear attributes and go to the next row
					// if a clear: right has
					...
					if (context.fitsOnCurrentRow(child)
							&& ((context.hasSavedForLaterChildren() == false) || (context
									.hasBeenSavedForLater(child)))) {
						System.out.println("PLACE ON CURRENT ROW!");
						context.placeOnCurrentRow(child);
						continue;
					}
					// If we can't fit it and we have more regular children,
					...
					// continue with the regular children
						...
						// No more regular elements left, so we must treat this
						// child
						// as a regular element and place it ... somewhere.
					}
				}

				// For regular (non-float) elements, try to place me on the
				// current row,
				...
				if (context.fitsOnCurrentRow(child)) {
					...
					// Make sure that I'm still the next child returned by
					// nextChild()
			...
			} else // If I am a block child, then I should be alone on my own
					// row ....
			if ("block".equals(childDisplayMode)
					|| "list-item".equals(childDisplayMode)) {
				if (child.clearRight) {
					if ((context.getRightFloatsCount()) > 0
							|| (context.getCurrentRowRightFloatOccupiedSpace() > 0)) {
						do {
							context.removeFloatsForBox(child);
							context.nextRow(1);
						} while ((context
				...
				if (child.clearLeft) {
					if ((context.getLeftFloatsCount()) > 0
							|| (context.getCurrentRowLeftFloatOccupiedSpace() > 0)) {
						do {
							context.removeFloatsForBox(child);
							context.nextRow(1);
		...
		// Is my content height pre-set or should I use my current height (i.e
		// Y-position )
		if (StyleManager.getProperty(box, "height") != null) {
			box.contentHeight = ((Dimension) StyleManager.getProperty(box,
					"height")).getValue(box.parent.contentHeight);
		} else {
		...
		if (box.hasDynamicSize == true) {
			if ("block".equals(displayMode) && "none".equals(floatMode)) {
				box.contentWidth = Math.max(context.getMaxX(), box
						.getContentWidthFromTotalWidth(parentContext
								.getRemainingRowWidth()));
			} else {
