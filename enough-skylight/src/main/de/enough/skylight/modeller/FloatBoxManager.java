...
public class FloatBoxManager {
	...
	public Float addFloat(int x, int y, int width, int height, int type, Box box) {
		...
		f.box = box;
		this.floats.add(f);
		System.out.println("FLOAT : " + x + " " + y + " " + width + " "
				+ height + " " + type);
		return f;
	...
	public int leftIntersectionPoint(int x, int y, int length, Box box) {
		...
		do {
			...
			i = 0;
			while (i < count) {
				temp = (Float) this.floats.get(i);
				if ((temp.y <= y)
						&& (temp.y + temp.height - 1 >= y)
						&& (((temp.x >= x) && (temp.x <= endX)) || ((temp.x
								+ temp.width > x) && (temp.x + temp.width < endX)))
		...
		do {
			...
			while (i < count) {
				...
				if ((temp.y <= y)
					...
					System.out.println("CHECK FLOAT: " + temp.x + " "
							+ tempLength);
					if (tempLength > maxX) {
						maxX = tempLength;
					}
	...
	public void removeFloatsForBox(Box b) {
		...
		while (i < size) {
			if (((Float) this.floats.get(i)).box == b) {
