...
//
///**
// * <p>In a first step, get the ordering of the tokens. In a second step maintain positions. A thirst step
// * is needed for maintaining relative positions to reduce overhead.</p>
// *
...
//	
//	
//	The idea is to match in one step hoping the regEx implementation construct
...
//		Do reset from the outside when something changed in the input.
...
//				int length = this.matcher.end() - this.matcher.start(); check if matcher.start is equal to currentPos.
...
//		int length = 0;
//		
//		Matcher matcher2 = pattern.matcher(this.stream); convert matcher variables to fields.
//		//matcher.reset(this.stream);Use matcher as field in conjunction with reset.
//		
...
//				length = matcher2.end() - matcher2.start(); check if matcher.start is equal to currentPos.
...
//	//Matcher nameMatcher = Pattern.matches()It would be nice if the matcher would be a field.
//	*/
//	
//	/**
