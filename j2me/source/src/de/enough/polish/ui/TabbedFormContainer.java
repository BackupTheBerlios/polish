 ...
 */
public class TabbedFormContainer extends Form {
	 ...
	 */
	public TabbedFormContainer(String title, String[] tabNames, Image[] tabImages ) {
	 ...
	 */
	public TabbedFormContainer(String title, String[] tabNames, Image[] tabImages, Style style) {
		super(title, style );
		this.tabTitles = new Container( true );
		this.tabTitles.screen = this;
		//#style tabbar, default
		UiAccess.setStyle(this.tabTitles);
		int length;
		if (tabNames != null) {
			length = tabNames.length;
			if (tabImages == null) {
				tabImages = new Image[length];
			}
		} else {
			length = tabImages.length;
			tabNames = new String[length];
		}
		for (int i = 0; i < length; i++) {
			//#style inactivetab, tab, default
			IconItem item = new IconItem(tabNames[i], tabImages[i]);
		...
		setSubTitle( this.tabTitles );
		this.currentlyActiveContainer = this.tabContainers[0];
	}
   ...
   * @param itemStyle the style for that item
	...
	public void setText(int tabIndex, String text ) {
		StringItem item = (StringItem) this.tabTitles.get(tabIndex);
		item.setText(text);
	}
	...
	protected boolean handleKeyPressed(int keyCode, int gameAction) {
		if (this.currentlyActiveContainer == this.tabTitles) {
		...
		else if (gameAction == Canvas.UP && this.container.focusedIndex == 0) {
			focus(this.tabTitles, true);
			this.tabTitles.focus(this.activeTabIndex);
			this.currentlyActiveContainer = this.tabTitles;
			return true;
		}
		//#if polish.Container.allowCycling
			else if (gameAction == Canvas.DOWN && this.container.focusedIndex == this.container.items.length - 1) {
				focus(this.tabTitles, true);
				this.tabTitles.focus(this.activeTabIndex);
				this.currentlyActiveContainer = this.tabTitles;
				return true;
...
//			System.err.println("Michael: handleKeyPressed: 2: focusedItem: " + focusedItem);
...
//				System.err.println("Michael: handleKeyPressed: 3");
//				if (focusedItem.internalX != -9999) {
//					this.container.scroll( gameAction, focusedItem );
//				}
//				return true;
//			}
////			System.err.println("Michael: handleKeyPressed: 2b");
////			else if ( this.tabTitles.handleKeyPressed(keyCode, gameAction)) {
////				System.err.println("Michael: handleKeyPressed: 4");
////				// TODO: Implement this.
////				notifyScreenStateChanged();
////				return true;
////			}
//		}
////		System.err.println("Michael: handleKeyPressed: 10: " + this.container.containerView.focusedIndex);
